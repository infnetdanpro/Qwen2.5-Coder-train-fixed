            ...
            self.logger.info(
                "".join(
                    [
                        ...
                        f"[epochs={logs['epoch']}]",
                        ",".join(
                            f"{k}={v}" for k, v in logs.items() if k not in self.exclude
                        ),
                    ]
                )
            )
...
class SaveDiskCallback(transformers.TrainerCallback):
    def on_save(self, args, state, control, **kwargs):
        ...
        for ckpt in os.listdir(args.output_dir):
            ...
            if ckpt.startswith("checkpoint-") and not ckpt.endswith(
                f"-{state.global_step}"
            ):
    ...
    def on_train_end(self, args, state, control, **kwargs):
        if state.is_local_process_zero:
            for pattern in ["global_step*", "*.pth"]:
                os.system(
                    "rm -rf " + os.path.join(args.output_dir, "checkpoint-*", pattern)
                )
