...
def chatml_format_preprocess(
    ...
    for j, sentence in enumerate(sources[1:]):
        ...
        if role == "<|im_start|>user" or (
            only_last_turn_loss and j < len(sources[1:]) - 1
        ):
            _target = (
                [im_start]
        ...
        ):  # if has more system prompt in the conversion
            _target = (
                [im_start]
                + [IGNORE_INDEX] * (len(_input_id) - 3)
                + [im_end]
    ...
    if return_test_input_ids:
        ...
        return dict(input_ids=input_id, label=target, length=[len(input_id)])
...
def read_file_from_position_with_chatml_format_processor(args):
    ...
    ) as f:  # Using 'replace' to handle errors better
...
def tokenize_file(
    ...
    output_objs = utils.multi_tasks_from_file(
        ...
        args={"tokenizer": tokenizer, "max_len": max_len},
    )
    if save_format == ".jsonl":
        ...
        save_mmap(
            output_objs,
            key="input_ids",
            output_path=f"{output_path}.input_ids.mmap",
            padding_value=tokenizer.pad_token_id,
        )
        save_mmap(
            output_objs,
            key="label",
            output_path=f"{output_path}.labels.mmap",
            padding_value=IGNORE_INDEX,
        )
        save_mmap(
            output_objs,
            key="length",
            output_path=f"{output_path}.lengths.mmap",
            padding_value=IGNORE_INDEX,
        )
        print(
            f"Successfully saved to {output_path}.input_ids.mmap and {output_path}.label.mmap and {output_path}.lengths.mmap"
        )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        "-output_path",
        type=str,
        default="./raw/sft.jsonl.sampled.processed",
        help="Path to output file",
    )
    parser.add_argument(
        "--workers", "-workers", type=int, default=1, help="Number of workers"
    )
    parser.add_argument(
        "--chunk_size",
        "-chunk_size",
        type=float,
        default=0.1 * 2**30,
        help="Chunk size for file processing",
    )
    parser.add_argument(
        "--max_len",
        "-max_len",
    ...
    parser.add_argument(
        ...
        "-tokenizer_path",
        type=str,
        default="./pretrained_models/qwen/Qwen2.5-Coder-7B/",
