...
def make_supervised_data_module(
    ...
    if args.data_path.endswith(".npy") or args.data_path.endswith(".jsonl"):
        train_dataset = training_datasets.SupervisedDataset(
            tokenizer=tokenizer, data_path=args.data_path, args=args
        )
    elif args.data_path.endswith(".mmap"):
        train_dataset = training_datasets.MMAPSupervisedDataset(
            tokenizer=tokenizer, data_path=args.data_path, args=args
        )
    ...
    def on_log(self, args, state, control, logs=None, **kwargs):
        if logs is not None:
            ...
            if args.include_num_input_tokens_seen:
                tokens_processed = (
                    state.num_input_tokens_seen - self.total_tokens
                ) * world_size
                ...
                tokens_processed = (
                    batch_size * max_seq_length * steps_elapsed * world_size
                )
                self.last_step = current_step
            time_elapsed = current_time - self.last_time
            tokens_per_second = (
                tokens_processed / time_elapsed if time_elapsed > 0 else 0
            )
            self.last_time = current_time
            log_message = {
...
def train():
    ...
    model = transformers.AutoModelForCausalLM.from_pretrained(
        ...
        attn_implementation=(
            "flash_attention_2" if model_args.use_flash_attention else None
        ),
