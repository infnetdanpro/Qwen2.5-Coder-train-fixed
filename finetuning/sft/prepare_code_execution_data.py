...
import tqdm
from utils import code_execute_multiple, utils
...
def execute_code_task(objs, worker_id=0, workers=1, args=None):
    ...
    for obj in tqdm.tqdm(objs, position=worker_id, desc=f"Worker {worker_id}"):
        ...
        if answer_match is not None and unittest_match is not None:
            ...
            unittest_code = remove_irrelevant_code(
                unittest_code, entry_point="check_correctness"
            )
            answer_code = answer_match.group(1)
            answer_code = pack_code(answer_code, programming_language)
            code = answer_code + "\n" + unittest_code
            if code_execute_multiple.check_correctness_multiple(
...
def parse_args():
    ...
    parser.add_argument(
        ...
        "-output_path",
        type=str,
        default="python_evol.jsonl.unittest",
        help="Path to output file",
    )
    parser.add_argument(
        "--workers", "-workers", type=int, default=1, help="Path to output file"
    )
...
def main():
    ...
    objs = utils.multi_tasks_from_objs(
        objs, workers=args.workers, task=execute_code_task, chunk_size=None, args=None
    )
