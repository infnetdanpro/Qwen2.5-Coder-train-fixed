...
def parse_args():
    ...
    parser.add_argument(
        "--input_path", "-input_path", default="./CodeArena_v1.jsonl", type=str, help=""
    )
    parser.add_argument(
        "--output_path",
        "-output_path",
        default="./results/yi-lightning/results.jsonl",
        type=str,
        help="",
    )
    parser.add_argument(
        "--model_max_len", "-model_max_len", default=8192 * 2, type=int, help=""
    )
    parser.add_argument(
        "--chat_template",
        "-chat_template",
...
def api_query(messages, model_name):
    if model_name == "yi-lightning":
        ...
        completion = client.chat.completions.create(
            model="yi-lightning", messages=messages
        )
...
def main():
    ...
    if args.model in api_models:
        ...
        if hasattr(tokenizer, "model_max_length"):
            args.model_max_len = (
                args.model_max_len
                if tokenizer.model_max_length > args.model_max_len
                else tokenizer.model_max_length
            )
            print(
                f"max_length: {tokenizer.model_max_length}, cur_length: {args.model_max_len}"
            )
        ...
        for obj in tqdm.tqdm(test_data):
            if args.chat_template == "codellama":
                ...
                obj["input"] = codellama_template.format_map(
                    {"system_prompt": "", "instruction": obj["messages"][-1]["content"]}
                )
            else:
                obj["input"] = tokenizer.apply_chat_template(
                    obj["messages"], add_generation_prompt=True, tokenize=False
                )
            objs.append(obj)
        sampling_params = vllm.SamplingParams(
            temperature=0.0, top_p=0.95, max_tokens=args.model_max_len
        )
        model = vllm.LLM(
            model=args.model,
            tensor_parallel_size=args.tensor_parallel_size,  # gpu_memory_utilization=0.95,
            worker_use_ray=True,
